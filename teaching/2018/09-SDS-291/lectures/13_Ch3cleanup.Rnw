...
<<eval=FALSE,  size='footnotesize', tidy=TRUE, echo=FALSE>>=
...
<<eval=FALSE, size='footnotesize', echo=FALSE>>=
require(openintro)
@
...
<<eval=FALSE, size='footnotesize', echo=FALSE>>=
math.paraellel <- makeFun(m.parallel)
math.indep <- makeFun(m.indep)
with(hsb2, plot3d(x = read, y = write, z = math, type = "s", size = 1.5,
    col = "blue", xlab = "Reading score", ylab = "Writing score",
    zlab = "Math score"))

coefs <- coef(m.parallel)
planes3d(coefs["read"], coefs["write"], -1, coefs["(Intercept)"], alpha = opacity,
col = "lightgray")
planes3d(coefs["read"], coefs["write"], -1, coefs["(Intercept)"] + coefs["sesmiddle"], alpha = opacity,
col = "lightgray")
planes3d(coefs["read"], coefs["write"], -1, coefs["(Intercept)"] + coefs["seshigh"], alpha = opacity,
col = "lightgray")

coefs <- coef(m.indep)
with(hsb2, plot3d(x = read, y = write, z = math, type = "s", size = 1.5,
    col = "blue", xlab = "Reading score", ylab = "Writing score",
    zlab = "Math score", xlim=c(0, 100), ylim=c(0,100), zlim=c(0,100)))
planes3d(coefs["read"], coefs["write"], -1, coefs["(Intercept)"], alpha = opacity,
col = "lightgray")
planes3d(coefs["read"]+coefs["read:sesmiddle"], coefs["write"]+coefs["write:sesmiddle"], -1, coefs["(Intercept)"] + coefs["sesmiddle"], alpha = opacity,
col = "lightgray")
planes3d(coefs["read"]+coefs["read:seshigh"], coefs["write"]+coefs["write:seshigh"], -1, coefs["(Intercept)"] + coefs["seshigh"], alpha = opacity,
col = "lightgray")
@
...
<<fig.show='hold', eval=TRUE, fig.height=3, message=FALSE, warning=FALSE, size='footnotesize'>>=
require(mosaic)
NYC <- read.csv("http://www.math.smith.edu/~bbaumer/mth241/nyc.csv")
...
y <- cos(3/4*pi*x) + rnorm(26, mean=0, sd=0.1)
d1 <- data.frame(y=y, x=x)
@

<<fig.width=3, fig.height=3, fig.show='hold', warning=FALSE>>=
